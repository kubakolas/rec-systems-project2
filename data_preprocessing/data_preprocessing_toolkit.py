...
class DataPreprocessingToolkit(object):
    ...
    def add_night_price(self, df):
        # Write your code here
        value = (df["accomodation_price"] / df["length_of_stay"] / df["n_rooms"])
        df.loc[:, "night_price"] = np.round(value, 2)
        return df
    ...
    def map_night_price_to_room_segment_buckets(self, df):
        df = df.copy()
        rooms_grouped = df.loc[df['night_price'] > 0].groupby('room_group_id')
        rooms_mean_prices = rooms_grouped['night_price'].mean().reset_index()
        columns = ['room_group_id', 'room_mean_price']
        rooms_mean_prices.columns = columns
        rooms_mean_prices = rooms_mean_prices[columns]
